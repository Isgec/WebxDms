Partial Class AF_xDmsFileTypes
  ...
  End Sub
  Protected Sub FVxDmsFileTypes_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles FVxDmsFileTypes.DataBound
    SIS.xDMS.xDmsFileTypes.SetDefaultValues(sender, e) 
  End Sub
  Protected Sub FVxDmsFileTypes_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles FVxDmsFileTypes.PreRender
    Dim oF_ReleaseWorkflowID_Display As Label  = FVxDmsFileTypes.FindControl("F_ReleaseWorkflowID_Display")
    Dim oF_ReleaseWorkflowID As TextBox  = FVxDmsFileTypes.FindControl("F_ReleaseWorkflowID")
    Dim oF_ReviseWorkflowID_Display As Label  = FVxDmsFileTypes.FindControl("F_ReviseWorkflowID_Display")
    Dim oF_ReviseWorkflowID As TextBox  = FVxDmsFileTypes.FindControl("F_ReviseWorkflowID")
    Dim mStr As String = ""
    Dim oTR As IO.StreamReader = New IO.StreamReader(HttpContext.Current.Server.MapPath("~/xDMS_Main/App_Create") & "/AF_xDmsFileTypes.js")
    mStr = oTR.ReadToEnd
    oTR.Close()
    oTR.Dispose()
    ...
    End If
    If Request.QueryString("FileTypeID") IsNot Nothing Then
      CType(FVxDmsFileTypes.FindControl("F_FileTypeID"), TextBox).Text = Request.QueryString("FileTypeID")
      CType(FVxDmsFileTypes.FindControl("F_FileTypeID"), TextBox).Enabled = False
    End If
  End Sub
