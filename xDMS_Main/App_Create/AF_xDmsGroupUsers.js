...
var script_xDmsGroupUsers = {
    ...
    validated_FK_xDMS_GroupUsers_UserID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_xDmsGroupUsers.validated_FK_xDMS_GroupUsers_UserID_main){
    ...
    validated_FK_xDMS_GroupUsers_GroupID: function(result) {
      var p = result.split('|');
      var o = $get(p[1]);
      if(script_xDmsGroupUsers.validated_FK_xDMS_GroupUsers_GroupID_main){
        var o_d = $get(p[1]+'_Display');
        try{o_d.innerHTML = p[2];}catch(ex){}
      }
      o.style.backgroundImage  = 'none';
