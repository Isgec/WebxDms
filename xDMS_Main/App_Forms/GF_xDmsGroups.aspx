<%@ Page Language="VB" MasterPageFile="~/MasterPage.master" AutoEventWireup="False" CodeFile="GF_xDmsGroups.aspx.vb" Inherits="GF_xDmsGroups" title="Maintain List: DMS Groups" %>
<asp:Content ID="CPHxDmsGroups" ContentPlaceHolderID="cph1" Runat="Server">
<div class="ui-widget-content page">
<div class="caption">
    <asp:Label ID="LabelxDmsGroups" runat="server" Text="&nbsp;List: DMS Groups"></asp:Label>
</div>
<div class="pagedata">
<asp:UpdatePanel ID="UPNLxDmsGroups" runat="server">
  <ContentTemplate>
    ...
    <LGM:ToolBar0 
      ID = "TBLxDmsGroups"
      ToolType = "lgNMGrid"
      EditUrl = "~/xDMS_Main/App_Edit/EF_xDmsGroups.aspx"
      AddUrl = "~/xDMS_Main/App_Create/AF_xDmsGroups.aspx?skip=1"
      ValidationGroup = "xDmsGroups"
      runat = "server" />
    <asp:UpdateProgress ID="UPGSxDmsGroups" runat="server" AssociatedUpdatePanelID="UPNLxDmsGroups" DisplayAfter="100">
      <ProgressTemplate>
    ...
    <asp:GridView ID="GVxDmsGroups" SkinID="gv_silver" runat="server" DataSourceID="ODSxDmsGroups" DataKeyNames="GroupID">
      <Columns>
        ...
        <asp:TemplateField HeaderText="Apply User">
          <ItemTemplate>
            <asp:ImageButton ID="cmdApply" ValidationGroup='<%# "Apply" & Container.DataItemIndex %>' CausesValidation="true" runat="server" Visible='<%# EVal("ApplyWFVisible") %>' Enabled='<%# EVal("ApplyWFEnable") %>' AlternateText='<%# EVal("PrimaryKey") %>' ToolTip="Apply User" SkinID="Forward" OnClientClick='<%# "return Page_ClientValidate(""Apply" & Container.DataItemIndex & """) && confirm(""Apply Group definition on users ?"");" %>' CommandName="ApplyWF" CommandArgument='<%# Container.DataItemIndex %>' />
          </ItemTemplate>
          <ItemStyle CssClass="alignCenter" />
          <HeaderStyle CssClass="alignCenter" Width="30px" />
        </asp:TemplateField>
    ...
    <asp:ObjectDataSource 
      ...
      SelectMethod = "UZ_xDmsGroupsSelectList"
      TypeName = "SIS.xDMS.xDmsGroups"
      SelectCountMethod = "xDmsGroupsSelectCount"
  ...
  <Triggers>
    <asp:AsyncPostBackTrigger ControlID="GVxDmsGroups" EventName="PageIndexChanged" />
